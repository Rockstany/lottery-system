...
if (!$communityId || !$userId) {
    header("Location: /public/group-admin/lottery/lottery.php?error=unauthorized");
    exit;
}
...
try {
    ...
    if ($checkStmt->rowCount() > 0) {
        header("Location: /public/group-admin/lottery/lottery.php?error=duplicate_request");
        exit;
    }
    ...
    header("Location: /public/group-admin/lottery/lottery.php?success=delete_requested");
    exit;

} catch (Exception $e) {
    error_log("Delete Request Error: " . $e->getMessage());
    header("Location: /public/group-admin/lottery/lottery.php?error=request_failed");
    exit;
}
