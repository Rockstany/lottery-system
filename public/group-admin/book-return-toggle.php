...
if (!$distributionId || !in_array($action, ['mark_returned', 'mark_not_returned'])) {
    header("Location: /public/group-admin/lottery-books.php?error=invalid");
    exit;
}
...
if (!$distribution) {
    header("Location: /public/group-admin/lottery-books.php?error=notfound");
    exit;
}
...
try {
    ...
    header("Location: /public/group-admin/lottery-books.php?error=failed");
    exit;
}
